...
function XWServer.Init()
	print(" XWServer Begin!(Default Server001)\r\n")
	--初始化世界
...
function XWServer.Enter(data)
	...
	--获得proxy代理xid链接
	print("GetProxy")
	local proxys = linker:GetProxy();
	--printTable(proxys)
	local proxymsg = {proxylist = proxys}--linkerlist = linkers
	XRPC.RPCCall(data.NetID, "XWClient.LinkerList", proxymsg)
end
